...
# Version:	19.10
# Author:	Remo Rickli
# Description:	Install NeDi 1.8C on Ubuntu or Debian
...
echo
read -r -p "Do you want netflow support? [y/n] " response
case "$response" in
    [yY][eE][sS]|[yY]) 
        apt install nfdump
	service nfdump stop
	rm -rf /var/cache/nfdump
	mkdir -p /var/nfdump/
	sed -i -e 's#-l /var/cache/nfdump#-e -w -y -Tall -M/var/nfdump#' /lib/systemd/system/nfdump.service
	nfexpire -s 8g -u /var/nfdump
	systemctl daemon-reload
	service nfdump start
	echo
	echo "Send netflow to this machine on UDP port 2055"
        ;;
    *)
        ;;
esac
echo "Finished!"
echo "Browse to https://this-machine and have fun!"
